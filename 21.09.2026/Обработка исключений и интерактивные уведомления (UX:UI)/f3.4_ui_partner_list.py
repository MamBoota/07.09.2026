...
# ==========================================
# НАСТРОЙКА ПУТЕЙ
# Файл лежит в: 21.09.2026/Обработка исключений и интерактивные уведомления (UX:UI)/
# ==========================================
...
def seed_test_data(db_path: str, stress_test: bool = False) -> None:
    if not is_db_empty(db_path) and not stress_test:
        return
...
def save_partner(db_path: str, partner_data: dict) -> None:
    """Сохраняет партнера в БД. Может выбросить sqlite3.Error при проблемах с БД."""
    conn = sqlite3.connect(db_path)
    try:
        cursor = conn.cursor()
...
# ==========================================
# ПЛЕЙСХОЛДЕР-ВИДЖЕТ С ПОДСВЕТКОЙ ФОКУСА
# ==========================================
class PlaceholderEntry(tk.Entry):
    def __init__(self, parent, placeholder: str, **kwargs):
        ...
        self.config(
            ...
            insertbackground=STYLE['text_primary'],
        )
...
class PartnerCard(tk.Frame):
    def __init__(self, parent, partner_data, on_edit_callback):
        ...
        hint = tk.Label(content, text="💡 Дважды кликните для редактирования",
                        font=('Segoe UI', 9, 'italic'), bg=STYLE['bg_card'],
                        fg='#999999', anchor='e')
    ...
    def _on_double_click(self, event):
        if self.on_edit_callback:
            self.on_edit_callback(self.partner_id)
...
class MainWindow(tk.Frame):
    ...
    def _create_header(self):
        ...
        tk.Button(hf, text="➕ Добавить партнера", font=STYLE['font_btn'],
                  ...
                  command=lambda: self.controller.show_frame("PartnerEditWindow", partner_id=None)).pack(side='right')
    ...
    def refresh_data(self):
        ...
        try:
            ...
            messagebox.showerror(
                ...
                "Пожалуйста, проверьте доступ к файлу partners.db и повторите попытку."
            )
            return
        ...
        for pid in ids:
            try:
                ...
                PartnerCard(self.cards_container, data, self._on_partner_edit).pack(fill='x', pady=15, padx=20)
            except Exception as e:
                print(f"Ошибка: {e}")

    def _on_partner_edit(self, partner_id: int):
        self.controller.show_frame("PartnerEditWindow", partner_id=partner_id)
...
class PartnerEditWindow(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self._initial_state = {}  # Состояние полей при открытии формы
        self._create_header()
        self._create_form()
    ...
    def _create_header(self):
        ...
        # Кнопка "Назад" с проверкой изменений
    ...
    def _create_form(self):
        ...
        # Кнопка "Отмена" с проверкой изменений
    ...
    def _on_resize(self, event):
        ...
        if w > 0:
            self.scroll_canvas.itemconfig(self.scroll_canvas_window, width=w)
    ...
    def _get_current_state(self) -> dict:
        """Возвращает текущее состояние всех полей формы."""
        state = {}
        for key, widget in self._fields.items():
            if key == 'partner_type':
                state[key] = widget.get()
            else:
    ...
    def _on_cancel(self):
        """Обработчик кнопок 'Назад' и 'Отмена' с проверкой изменений."""
        if self._has_changes():
            result = messagebox.askyesno(
                "Подтверждение выхода",
                "Вы изменили данные, но не сохранили их.\n\n"
                "Все несохранённые изменения будут потеряны.\n"
                "Вы уверены, что хотите вернуться назад?",
                icon='warning'
            )
            if not result:
                return  # Пользователь нажал "Нет" — остаёмся на форме
        
        self.controller.show_frame("MainWindow")

    def reset_form(self, partner_id: int = None):
        """Подготавливает форму и сохраняет начальное состояние."""
    ...
    def _validate_rating(self, rating_str: str) -> tuple[bool, int, str]:
        """Валидирует рейтинг. Возвращает (успех, значение, сообщение об ошибке)."""
        if not rating_str:
            return True, 0, ""  # Пустой рейтинг = 0
        ...
        try:
            ...
            if rating < 0:
                return False, 0, (
                    ...
                    "Пожалуйста, введите целое число от 0 или больше."
                )
            return True, rating, ""
        except ValueError:
            return False, 0, (
                "Рейтинг должен быть целым числом.\n\n"
                "Вы ввели некорректные символы. Пожалуйста, удалите знаки препинания, "
                "буквы и другие символы, оставив только цифры (например: 8)."
            )

    def _on_save(self):
        """Сохраняет партнера с полной валидацией и обработкой ошибок."""
        ...
        if not name:
            messagebox.showerror(
                ...
                "Пожалуйста, введите название организации или ФИО индивидуального предпринимателя."
            )
        ...
        if not email:
            messagebox.showerror(
                ...
                "Пожалуйста, введите адрес электронной почты (например: example@company.ru)."
            )
            self._fields['email'].focus_set()
            return

        # === ВАЛИДАЦИЯ РЕЙТИНГА через try/except ===
        is_valid, rating, error_msg = self._validate_rating(rating_str)
        if not is_valid:
            messagebox.showerror("Ошибка валидации", error_msg)
            self._fields['rating'].focus_set()
            return

        # === СОХРАНЕНИЕ В БД с обработкой исключений ===
        try:
            ...
            messagebox.showerror(
                "Ошибка базы данных",
                f"Не удалось сохранить данные в базу:\n{str(e)}\n\n"
                "Возможные причины:\n"
            ...
            messagebox.showerror(
                ...
                f"Произошла непредвиденная ошибка при сохранении:\n{str(e)}\n\n"
                "Пожалуйста, попробуйте ещё раз или обратитесь к администратору."
            )
            return

        # === УСПЕШНОЕ СОХРАНЕНИЕ — Information MessageBox ===
        mode = "обновлён" if self._editing_id else "добавлен"
        messagebox.showinfo(
            "Операция выполнена успешно",
            f"Партнер '{name}' успешно {mode} в базе данных.\n\n"
            "Сейчас вы будете возвращены к списку партнеров."
        )
        self.controller.show_frame("MainWindow")

# ==========================================
# КОНТРОЛЛЕР С ГЛОБАЛЬНЫМ СКРОЛЛОМ
# ==========================================
class CRMApp(tk.Tk):
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == '--stress':
        ...
        print("Запуск CRM (Этап 3, Задача 4 — Обработка исключений и уведомления)...")
        CRMApp().mainloop()