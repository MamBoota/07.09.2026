...
def extend_db_schema(db_path: str) -> None:
    """Добавляет колонки для формы редактирования, если их нет."""
...
class PlaceholderEntry(tk.Entry):
    """Entry с плейсхолдером и визуальной подсветкой рамки при фокусе."""
    def __init__(self, parent, placeholder: str, **kwargs):
        ...
        # Начальное состояние: серая рамка
        self.config(
            highlightthickness=1,
            highlightbackground='#DDDDDD',
            highlightcolor='#4CAF50',
            insertbackground=STYLE['text_primary'],  # цвет мигающего курсора |
        )
    ...
    def _on_focus_in(self, _event):
        # Подсвечиваем рамку зелёным при фокусе
...
class MainWindow(tk.Frame):
    ...
    def _create_content_area(self):
        ...
        # ВАЖНО: bind_all убран отсюда — теперь скролл управляется глобально в CRMApp
...
class PartnerEditWindow(tk.Frame):
    ...
    def _create_form(self):
        ...
        # bind_all убран — скролл управляется глобально
    ...
    def _on_save(self):
        ...
        mode = "обновлен" if self._editing_id else "добавлен"
        messagebox.showinfo("Успех", f"Партнер успешно {mode}!")
...
class CRMApp(tk.Tk):
    def __init__(self, stress_test: bool = False):
        ...
        # === ГЛОБАЛЬНЫЙ СКРОЛЛ КОЛЕСОМ МЫШИ ===
        # Работает на всех экранах, скроллит только активный canvas
        self.bind_all("<MouseWheel>", self._on_mousewheel)
        # Поддержка Linux (кнопки 4/5)
    ...
    def _find_canvas_in(self, widget):
        """Рекурсивно находит скроллящийся Canvas внутри виджета."""
    ...
    def _on_mousewheel(self, event):
        """Глобальный обработчик скролла для macOS/Windows."""
    ...
    def _on_mousewheel_step(self, steps):
        """Глобальный обработчик скролла для Linux."""
        if self._current_page and self._current_page in self.frames:
            canvas = self._find_canvas_in(self.frames[self._current_page])
            if canvas:
    ...
    def show_frame(self, page_name, partner_id=None):
        ...
        frame.tkraise()
        self._current_page = page_name  # Запоминаем активную страницу для скролла
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == '--stress':
        ...
        print("Запуск CRM (Этап 3, Задача 3 — CRUD и обновление UI)...")
        CRMApp().mainloop()